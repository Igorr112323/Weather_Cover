 ...
 *
 * Защита (подробности — docs/ЗАЩИТА.md):
 *   - при запуске проверяются условия запуска — отладчик, --inspect,
 *     переменные разработки (shield.cjs);
 ...
 * уходит обфусцированная копия electron/ (scripts/harden.mjs).
 */
...
const { decideLaunch, hardenWebContentsAgainstDebugging } = require("./shield.cjs");
...
/**
 * Каналы данных доступны только главному окну: ни одно другое содержимое
 * (в том числе открытое через window.open) до них не дотянется.
 */
...
/**
 * Пропуск к данным: канал работает только для главного окна. Возвращает null,
 * если доступ разрешён, иначе готовый ответ.
 */
function gateDataAccess(event) {
...
function registerIpc() {
  ...
  ipcMain.handle("agro:read-database", async (event, payload) => {
    const denied = gateDataAccess(event);
...
if (!gotLock) {
  ...
  void app.whenReady().then(async () => {
    // Разрешения: по умолчанию всё отклоняется.
