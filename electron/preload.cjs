 ...
 * выгрузки (путь выбирает человек в системном диалоге), — и два односторонних
 * сигнала: «интерфейс построен» для экрана загрузки и «под кнопками окна
 * тёмное содержимое» для цвета их значков.
 */
...
contextBridge.exposeInMainWorld("agro", {
  ...
  async writeDatabase(bytes) {
    ...
    return invoke("agro:write-database", { bytes: payload });
  },

  async preserveDatabase(reason) {
    return invoke("agro:preserve-database", { reason: toReason(reason) });
  },
