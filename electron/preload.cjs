 ...
 * Наружу отдаются только четыре операции с данными, каждая с проверкой
 * аргументов: чтение базы, запись базы, сохранение копии повреждённой базы,
 * запись файла выгрузки (путь выбирает человек в системном диалоге), —
 * и два односторонних сигнала: «интерфейс построен» для экрана загрузки
 * и «под кнопками окна тёмное содержимое» для цвета их значков.
 */
