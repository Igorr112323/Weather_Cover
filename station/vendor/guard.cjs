/* vendored: точная копия electron/license/guard.cjs из проекта Weather_Cover.
 ...
 * @param {string} [options.licenseFileName] имя файла лицензии (проверочный
 *        режим передаёт своё — см. license/checkmode.cjs)
...
function createGuard(options = {}) {
  ...
  const store =
    ...
    createLicenseStore({
      dir: userDataDir,
      // Имя файла лицензии меняет проверочный режим (license/checkmode.cjs):
      // у проверочной копии свой файл, поэтому она обязана спросить код заново.
      fileName: options.licenseFileName ?? "agroprognoz.license",
