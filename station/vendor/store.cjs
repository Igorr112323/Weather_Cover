...
/* ── Разбор и проверка конверта (без расшифровки) ────────────────────────── */

/** Каноническая строка для HMAC: порядок полей зафиксирован версией конверта. */
function canonicalEnvelope(envelope) {
  return [envelope.v, envelope.cipher, envelope.salt, envelope.iv, envelope.tag, envelope.data, envelope.machine].join("|");
}

function envelopeMacFor(machineId, envelope) {
  const key = deriveKey(String(machineId ?? ""), fromBase64(envelope.salt), HKDF_INFO_MAC);
  return crypto.createHmac("sha256", key).update(canonicalEnvelope(envelope), "utf8").digest();
}

/** Отпечаток компьютера в открытой части конверта: перенос файла на другую машину виден сразу. */
function machineTag(machineId) {
  return toBase64(crypto.createHash("sha256").update(String(machineId ?? ""), "utf8").digest());
}

/**
 * Проверка текста файла лицензии: формат, «свой» ли компьютер, не подменены ли поля.
 * Расшифровки здесь нет намеренно — так этот же код может осмотреть файл
 * diagnostic-скрипт (`npm run license:status`), у которого нет доступа к
 * системному хранилищу ключей.
 *
 * @param {string} machineId идентификатор этого компьютера
 * @param {string} raw содержимое файла (текст)
 * @returns {{ok:boolean, reason?:string, envelope?:object, cipher?:string, machineMatches?:boolean}}
 */
function inspectEnvelope(machineId, raw) {
  const text = String(raw ?? "").replace(new RegExp(`^${MAGIC}\\s*`), "").trim();
  let envelope;
  try {
  ...
  if (!envelope || typeof envelope !== "object" || envelope.magic !== MAGIC || envelope.v !== ENVELOPE_VERSION) {
    return { ok: false, reason: "FORMAT" };
  }
  const info = { envelope, cipher: String(envelope.cipher ?? ""), machineMatches: String(envelope.machine ?? "") === machineTag(machineId) };
  if (!info.machineMatches) return { ...info, ok: false, reason: "MACHINE" };
  const expected = toBase64(envelopeMacFor(machineId, envelope));
  const actual = String(envelope.mac ?? "");
  if (expected.length !== actual.length || !crypto.timingSafeEqual(Buffer.from(expected, "base64"), Buffer.from(actual, "base64"))) {
    return { ...info, ok: false, reason: "MAC" };
  }
  return { ...info, ok: true };
}
...
function createLicenseStore({ dir, fileName = "agroprognoz.license", machineId, safeStorage, log = () => {} }) {
  ...
  function envelopeMac(envelope) {
    return envelopeMacFor(machine(), envelope);
  }
  ...
  async function read() {
    ...
    const inspected = inspectEnvelope(machine(), raw);
    if (!inspected.ok) return { status: STATUS.INVALID, reason: inspected.reason ?? "FORMAT" };
