...
/**
 * Мини-DOM: ровно то, чем пользуется скрипт страницы. Нужен, чтобы проверить
 * связку «кнопка → обработчик → результат», а не только чистую логику: опечатка
 * в id или отсутствующий элемент иначе всплывёт только в браузере.
 */
